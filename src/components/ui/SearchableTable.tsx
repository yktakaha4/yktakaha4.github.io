...
export const createSearchRegexp = (query: string) => {
  const conditions = query.trim().normalize(normalizeForm).split(/\s+/);
...
export const createSearchText = (...texts: Array<string>) => {
  return texts
    .map((t) => t.trim().normalize(normalizeForm).replace(/\s+/g, '\x20'))
    .join('\x20');
};
...
export const SearchableTable = <T,>({
  ...
  const filtered = items.filter((item) =>
    regexp.test(createSearchText(...searchTexts(item))),
  );
  const rows =
    filtered.length > 0
      ? filtered
